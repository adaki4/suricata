...
static void HandleShutdown(DPDKThreadVars *ptv)
{
    ...
    if (ptv->queue_id == ptv->threads - 1) {
        ...
        if (SC_ATOMIC_GET(ptv->livedev->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_active) != 0) {
            SCLogInfo("Waiting for all bypass rte_flow rules to be removed");
            while (SC_ATOMIC_GET(ptv->livedev->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_active) != 0) {
                rte_delay_us(100000);
                SCLogInfo("rules added %d, rules left to remove %d", 
                    SC_ATOMIC_GET(ptv->livedev->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_created), 
                    SC_ATOMIC_GET(ptv->livedev->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_active));
            }
        }
        StatsSetUI64(ptv->tv, ptv->capture_dpdk_rules_created, SC_ATOMIC_GET(ptv->livedev->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_created));
        DPDKDumpCounters(ptv);
...
static TmEcode ReceiveDPDKThreadInit(ThreadVars *tv, const void *initdata, void **data)
{
    ...
    ptv->capture_dpdk_rules_created =
            StatsRegisterCounter("capture.dpdk.rules_created", ptv->tv);
