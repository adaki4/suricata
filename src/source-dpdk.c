        ...
        DPDKThreadVars *ptv, DPDKIfaceConfig *dpdk_config, const char *driver_name)
{
    ...
    if ((strcmp(driver_name, "mlx5_pci") == 0 || strcmp(driver_name, "net_ice") == 0 ||
        ...
        ptv->livedev->dpdk_vars.rte_flow_rule_handlers =
                dpdk_config->drop_filter.rte_flow_rule_handlers;
        dpdk_config->drop_filter.rte_flow_rule_handlers = NULL;
        ptv->livedev->dpdk_vars.rte_flow_rule_count =
                dpdk_config->drop_filter.curr_rte_flow_rule_count;
    }
    SCReturnInt(0);
}
...
static inline void DPDKDumpCounters(DPDKThreadVars *ptv)
{
    ...
    if (ptv->queue_id == ptv->threads - 1) {
        ...
        if (!ptv->port_stopped) {
            ...
            retval = QueryRteFlowFilteredPackets(ptv->livedev->dpdk_vars.rte_flow_rule_handlers,
                    ptv->livedev->dpdk_vars.rte_flow_rule_count, ptv->livedev->dev, ptv->port_id,
                    &filtered_packets);
            if (retval == 0)
...
static inline Packet *PacketInitFromMbuf(DPDKThreadVars *ptv, struct rte_mbuf *mbuf)
{
    ...
    p->dpdk_v.in_port_id = ptv->port_id;
