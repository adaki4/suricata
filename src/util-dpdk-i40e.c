        ...
        int port_id, const char *port_name, struct rte_flow_action_rss rss_conf)
{
    ...
    ret |= DPDKCreateRSSFlow(port_id, port_name, rss_conf, RTE_ETH_RSS_IPV4, pattern);
        ...
        int port_id, const char *port_name, struct rte_flow_action_rss rss_conf)
{
    ...
    pattern[2].type = RTE_FLOW_ITEM_TYPE_END;
    ret |= DPDKCreateRSSFlow(port_id, port_name, rss_conf, RTE_ETH_RSS_IPV6, pattern);
    memset(pattern, 0, sizeof(pattern));
...
static int i40eDeviceSetRSSWithFlows(int port_id, const char *port_name, int nb_rx_queues)
{
    ...
    rss_action_conf = DPDKInitRSSAction(rss_conf, 0, queues, RTE_ETH_HASH_FUNCTION_TOEPLITZ, true);
