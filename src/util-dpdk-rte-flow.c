...
static bool RteFlowDropFilterInit(uint32_t rule_count, char **patterns, struct rte_flow_attr *attr,
        struct rte_flow_action *action, const char *driver_name, const char *port_name)
{
    attr->ingress = 1;
    attr->priority = 0;
        ...
        const char *driver_name)
{
    ...
    bool should_gather_stats =
            RteFlowDropFilterInit(rte_flow_rule_storage->curr_rte_flow_rule_count,
                    rte_flow_rule_storage->rte_flow_rules, &attr, action, driver_name, port_name);

    rte_flow_rule_storage->rte_flow_rule_handlers = SCMalloc(
            sizeof(struct rte_flow_rule *) * rte_flow_rule_storage->curr_rte_flow_rule_count);
