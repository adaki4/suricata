...
int RteBypassInit(DPDKDeviceResources *dpdk_resources, uint32_t bypass_ring_size, const char *port_name, int port_id)
{
    ...
    struct rte_mempool *bypass_mp = rte_mempool_create(RTE_BYPASS_MEMPOOL_NAME,
        ...
        ThreadVars *th_v, struct flows_stats *bypassstats, struct timespec *curtime, void *data)
{
    ...
    uint32_t to_bypass_packets = rte_ring_dequeue_burst(bypass_ring, (void **)ring_data, ring_dequeue_num, NULL);
    for (uint16_t i = 0; i < to_bypass_packets; i++) {
        ...
        if (retval != 0) {
            SC_ATOMIC_ADD(flow->livedev->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_active, 1);
            SC_ATOMIC_ADD(flow->livedev->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_created, 1);
            bypassstats->count++;
            success_count++;
        }
        FLOWLOCK_UNLOCK(flow);
...
bool RteBypassUpdate(Flow *flow, void *data, time_t tsec)
{
    ...
    if (!activity || unlikely(suricata_ctl_flags != 0)) {
        if (flow_handler_info->src_handler != NULL && flow_handler_info->dst_handler != NULL) {
            ...
            SC_ATOMIC_SUB(flow_handler_info->dpdk_vars->rte_flow_bypass_data->rte_bypass_rules_active, 1);
        }
    }
