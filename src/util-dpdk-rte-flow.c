        ...
        ThreadVars *th_v, struct flows_stats *bypassstats, struct timespec *curtime, void *data)
{
    ...
    for (uint16_t i = 0; i < to_bypass_packets; i++) {
        ...
        const char *livedev_name = LiveGetDeviceName(flow_key->livedev_id);
        LiveDevice *livedev = LiveGetDevice(live_dev_name);
        if (flow == NULL) {
        ...
        if (FLOW_IS_IPV4(flow)) {
            ...
            for (uint8_t i = 0; i < 16; i++) {
                ipv6_spec.hdr.src_addr[i] = flow_key->src.address.address_un_data8[i];
                ipv6_mask.hdr.src_addr[i] = 0xFF;
                ipv6_spec.hdr.dst_addr[i] = flow_key->dst.address.address_un_data8[i];
        ...
        if (FLOW_IS_IPV4(flow)) {
            ...
            SCLogDebug("Add an IPv6 rte_flow bypass rule");
            for (uint8_t i = 0; i < 16; i++) {
                ipv6_spec.hdr.src_addr[i] = flow_key->dst.address.address_un_data8[i];
                ipv6_mask.hdr.src_addr[i] = 0xFF;
                ipv6_spec.hdr.dst_addr[i] = flow_key->src.address.address_un_data8[i];
                ipv6_mask.hdr.dst_addr[i] = 0xFF;
            }
            ip_spec = &ipv6_spec;
            ip_mask = &ipv6_mask;
